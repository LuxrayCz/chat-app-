...
const page = async ({ params }: pageProps) => {
  ...
  const chatPart = (await fetchRedis("get", `user:${chatPartnerId}`)) as string;
        ...
        <div className="relative flex items-center space-x-4">
          <div className="relative">
            <div className="relative w-8 sm:w-12 h-8 sm:h-12">
              <Image src={chatPartner.image} fill referrerPolicy="no-referrer" alt={`${chatPartner.name} profile picture`} className="rounded-full" />
            </div>
          </div>
